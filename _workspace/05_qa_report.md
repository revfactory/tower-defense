...
**[회차 57-보강] team-lead 6항목 재검증 — 현재 타일(mtime 20:05:32, 회차57 이후 재저장) 기준 GREEN.** ③⑥ 추가 확인:
- **③ 도로 본체 불투명 유지(밑 잔디 비침 없음):** 도로 코어 알파 정밀 측정 — 직선(h·v) 중앙밴드 **min α255·100% 불투명**, 코너(ne/se) 중앙영역 mean α254·94-96% 불투명(잔여 semi는 안쪽 곡선 엣지, 도로 관통 투명 홀 0). 밑 잔디가 도로 뚫고 비치지 않음
- **⑥ 부팅 스모크 콘솔 0:** 캐시무효 새 탭 9222→8901 부팅+스테이지 진입(tilemap 길 타일 실렌더 경로) → 콘솔 총 2줄(info)·**error/exception 0·"레이어 draw 예외" 0·"계약 외 레이어" 0**
- ①②④⑤ 현재 타일 재확인: 재렌더(투명 프로브 42%=신 타일)에서 사각 얼룩 0(stage4 파생 se/sw U턴 연속)·se=vflip(ne)/sw=vflip(nw) 100%·순수잔디 ΔE 회귀 0(6.6/6.3/4.7/2.5/1.9 초과0)·256²/키/경로/매니페스트/엔진 불변·백업 존치

**회차 57 최종 = GREEN(6/6).** 하네스 §7에 규칙 반영됨(오버레이 타일 여백 투명·edge는 컷 금지·저품질 코너 형제 플립). **주의(#14 대비):** 하네스 §7이 "edge 전이 타일(물가·흙가)은 알파 컷 금지"(밑에 깔리는 게 잔디 아닌 물/흙이라 그라디언트 깨짐)를 명문화 — #14가 water_edge·dirt_edge에 path식 알파컷을 그대로 적용하면 이 규칙과 상충하므로, #14 검증 시 컷이 물/흙 그라디언트를 깨지 않는지 집중 확인 필요.
