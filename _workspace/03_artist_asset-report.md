...
| 6 (v5.2) | 2026-07-19 | 길 타일 잔디 여백 알파 도려내기(6종) — 스티커 제거, 여백=실제 잔디 필드 비침(ΔE≈0). se·sw는 형제 ne·nw 수직플립 파생(저품질 원본 교정). 렌더 5스테이지 코너 클린 검증. 알파 추가 외 키·경로·규격·코드 불변. QA·playtester GREEN(회차 57). |

### #14 전이 타일(water_edge·dirt_edge) 알파 컷 — 조사 후 종료(부적용)

**결론: 알파컷 부적용, 작업 안 함(team-lead 종료 판정).** 근거:
- **코드(src/map/tilemap.js):** 물/흙 셀 draw 순서 = `grass_base(263) → tile_water/dirt(271) → edge 타일(279, 회전)`. edge 잔디 여백 바로 밑은 **패밀리 타일(물/흙)**이라, 길 타일과 달리 컷하면 grass가 아니라 물/흙이 드러나 grass→물/흙 전이 그라디언트(edge 타일 존재 이유)가 파괴된다.
- **렌더 증거:** `_workspace/03_{water,dirt}_edge_render.png` — v5 세그먼트 하모나이즈(water_edge 잔디 d=7.2, dirt_edge d=8.4)로 이미 전이가 자연스럽고 뚜렷한 스티커 없음.
- **처방(후속용):** 만약 향후 플레이테스트에서 물/흙 경계에 미세 seam이 잡히면, 해법은 알파컷이 아니라 **edge 타일 잔디부의 더 강한 분포 하모나이즈(v5.1 방식, JND급)**다. 즉시 가능하나 현재 불필요.

**#13 잔여 관찰(QA, 비블로커):** 코너 근접 시 옅은 초록 haze 사각 윤곽 ~4-5%(자연스러운 도로 엣지 페더). 더 낮추면 도로 엣지를 잠식하므로 현 수준 유지가 적정 — 수용 가능 잔차.
